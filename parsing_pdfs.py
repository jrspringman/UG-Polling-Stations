#Code adapted from ijmbarr at github.com/ijmbarr/parsing-pdfs/blob/master/PDFFixup/fixer.py
...
file_path = "2011_Pres_Pstn.pdf"
extracted_table = get_tables(file_path)

extracted_table[2]

extracted_table = [[[x.replace(u',', u'') for x in l] for l in y] for y in extracted_table]
extracted_table = [[[x.replace(u'\xa0', u' ') for x in l] for l in y] for y in extracted_table]
extracted_table = [[[x.replace(u'\u2010', u'-') for x in l] for l in y] for y in extracted_table]
...
open("example_out.csv", "wb").write(csved.encode("utf-8"))
#output = open('2K11.pkl', 'wb')
#pickle.dump(csved, output)
